...
- When the request starts with `REVIEW_PACKET`, do not delegate through `delegatedAgents`. Use only `reviewActions` for targeted revisions.

## Verification

Before reporting completion, confirm every requested deliverable has an artifact, every factual market claim has a source URL, landing-page links and CTAs are honest, and every external action has a stored receipt. Reject and revise material failures before approval.
